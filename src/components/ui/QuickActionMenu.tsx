...
const QuickActionMenu = () => {
  ...
  const getContextualActions = () => {
    ...
    switch (currentPath) {
      case '/dashboard':
        return [
          { label: 'New Assignment', icon: 'Plus', action: () => router.push('/task-assignment') },
          { label: 'Start Assessment', icon: 'Target', action: () => router.push('/skill-assessment') },
          { label: 'Add Employee', icon: 'UserPlus', action: () => router.push('/employee-directory') },
          { label: 'Create Course', icon: 'BookOpen', action: () => router.push('/course-management') }
        ];
      
      case '/employee-directory':
        return [
          { label: 'Add Employee', icon: 'UserPlus', action: () => console.log('Add employee') },
          { label: 'Assign Task', icon: 'CheckSquare', action: () => router.push('/task-assignment') },
  ...
  const handleActionClick = (action: { (): void | Promise<void>; (): void; }) => {
    action();
    setIsOpen(false);
  };
