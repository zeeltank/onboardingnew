...
import ViewKnowledge from "./viewDialouge"; // Adjust path as needed
...
const TreeView: React.FC<TreeViewProps> = ({ allSkillData }) => {
  ...
  const handleAddClick = async (skillname: string | null) => {
    // alert(skillname);
    try {
      const res = await fetch(`${sessionUrl}/knowledge_library`, {
        method: "POST",
        headers: {
