...
import ViewKnowledge from "./viewDialouge";
...
const AddSkillView: React.FC<AddSkillViewProps> = ({ userSkillsData, category, subCategory  }) => {
      ...
      const fetchData = async () => {
        try {
          const res = await fetch(
            `${sessionData.url}/knowledge_library?type=API&token=${sessionData.token}&sub_institute_id=${sessionData.subInstituteId}&org_type=${sessionData.orgType}&category=${category}&sub_category=${subCategory}`
          );
  ...
  const handleDelete = async () => {
    ...
    if (window.confirm("Are you sure you want to delete this job role?")) {
      try {
        const res = await fetch(
          `${sessionData.url}/knowledge_library/${selectedSkill}?type=API&token=${sessionData.token}&sub_institute_id=${sessionData.subInstituteId}&org_type=${sessionData.orgType}&user_id=${sessionData.userId}&formType=user`,
          {
            method: "DELETE",
            headers: {
  ...
  return (
    <div className="flex gap-4 mx-2 mb-2">
            ...
            <li>
              <summary className="cursor-pointer flex items-center p-0 hover:bg-gray-100 rounded">
                <span className="flex items-center font-semibold border-1 border-[#ddd] rounded-sm px-2">
                  <i className="mdi mdi-folder mr-2 font-bold text-yellow-700"></i> All Knowledge
                  <div className="openClose pl-4">
                    <i className={`mdi mdi-plus-box m-2 text-xl ${isOpen ? "text-cyan-600" : ""}`} onClick={() => toggleAll(true)} title="All Knowledge Open"></i>
                    <i
                      className={`mdi mdi-minus-box text-xl ${isOpen ? "" : "text-cyan-600"}`}
                      onClick={() => toggleAll(false)}
                      title="All Knowledge Close"
                    ></i>
                  </div>
                </span>
              </summary>
      ...
      {dialogOpen.view && selectedSkill && (
        <ViewKnowledge knowledgeId={selectedSkill}
          onClose={() => {
